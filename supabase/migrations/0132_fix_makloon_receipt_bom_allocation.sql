-- ============================================================
-- 0132_fix_makloon_receipt_bom_allocation.sql
-- Makloon Receipt: use BOM-based material cost allocation
-- No manual material cost input for receipt posting
-- ============================================================
...
begin
  ...
  if not found then
    raise exception 'Akun Bahan di Vendor Konveksi (1320) tidak ditemukan';
  end if;
  ...
  loop
    ...
    if v_new_qty > 0 then
      ...
      v_new_avg := case
        when v_item.qty_received > 0 then v_item_total_cost / v_item.qty_received
        else 0
      end;
    end if;
