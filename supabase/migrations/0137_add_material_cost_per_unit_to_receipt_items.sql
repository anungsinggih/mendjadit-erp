-- ============================================================
-- 0137_add_material_cost_per_unit_to_receipt_items.sql
-- Add missing material_cost_per_unit column to makloon_receipt_items
-- ============================================================

alter table public.makloon_receipt_items
add column if not exists material_cost_per_unit numeric(14,4) not null default 0;

alter table public.makloon_receipt_items
add column if not exists total_cost_per_unit numeric(14,4) generated always as (jasa_per_unit + material_cost_per_unit) stored;

alter table public.makloon_receipt_items
drop constraint if exists ck_makloon_receipt_item_nonneg;

alter table public.makloon_receipt_items
add constraint ck_makloon_receipt_item_nonneg check (
  qty_received > 0 and
  jasa_per_unit >= 0 and
  material_cost_per_unit >= 0 and
  subtotal_jasa >= 0
);

-- Update existing rows to set material_cost_per_unit = 0
update public.makloon_receipt_items
set material_cost_per_unit = 0
where material_cost_per_unit is null;

-- Update RPCs that reference makloon_receipt_items
...
-- Update rpc_post_makloon_receipt to use material_cost_per_unit
...
begin
  ...
  loop
    ...
    -- Calculate BOM-based material cost
    v_item_material_cost := 0;
    v_total_material := 0;

    -- Get BOM for this FG
    for v_bom in
      select ib.raw_material_id, ib.qty_per_fg, mii.material_cost_per_unit as material_cost
      from public.item_boms ib
      join public.makloon_material_issues mi on mi.makloon_order_id = v_receipt.makloon_order_id
      join public.makloon_issue_items mii on mii.issue_id = mi.id and mii.item_id = ib.raw_material_id
      where ib.finished_good_id = v_item.item_id
        and mi.status = 'POSTED'
    loop
      -- Calculate material cost for this FG based on BOM
      v_bom_qty := v_bom.qty_per_fg * v_item.qty_received;
      v_bom_material_cost := v_bom.material_cost * v_bom_qty;

      -- Add to total material cost for this receipt
      v_total_material := v_total_material + v_bom_material_cost;

      -- Add to item material cost
      v_item_material_cost := v_item_material_cost + v_bom_material_cost;
    end loop;

    -- Calculate total cost for this FG item
    ...
    -- Journal: Cr WIP - only if material cost exists
