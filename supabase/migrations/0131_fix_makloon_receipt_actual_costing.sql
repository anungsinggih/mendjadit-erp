-- ============================================================
-- 0131_fix_makloon_receipt_actual_costing.sql
-- Makloon Receipt: use actual material cost from posted material issues
...
begin
  ...
  select coalesce(sum(mii.subtotal), 0)
  into v_total_material
  from public.makloon_material_issues mi
  join public.makloon_issue_items mii on mii.issue_id = mi.id
  ...
  loop
    ...
    if v_total_material > 0 and v_total_received_qty > 0 then
      v_item_material_cost := round((v_item.qty_received / v_total_received_qty) * v_total_material, 2);
    end if;
